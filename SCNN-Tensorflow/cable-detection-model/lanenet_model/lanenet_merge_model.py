...
class LaneNet(cnn_basenet.CNNBaseModel):
    ...
    def test_inference(input_tensor, phase, name):
        ...
        with tf.variable_scope(name):
            ...
            binary_seg_ret = tf.nn.softmax(logits=decode_logits)# 把prob map softmax了
            prob_list = []
            kernel = tf.get_variable('kernel', [9, 9, 1, 1], initializer=tf.constant_initializer(1.0 / 81),
                                     trainable=False)# 不可train
            ...
            for cnt in range(1, binary_seg_ret.get_shape().as_list()[3]):
                print('binary_seg_ret.get_shape().as_list()[3]: ### ', binary_seg_ret.get_shape().as_list()[3])## 5
            ...
            # Predict lane existence: 车线检测的时候可以去除，因为图片里面必定会有车线出现
    ...
    def loss(inference, binary_label, existence_label, name):
        ...
        with tf.variable_scope(name):
            ...
            decode_logits = inference_ret['prob_output']#prob map
            decode_logits_reshape = tf.reshape(
                ...
                shape=[decode_logits.get_shape().as_list()[0],
                       ...
                       decode_logits.get_shape().as_list()[3]]) #batch x w x h x deep ?x36x100x5
            ...
            binary_label_reshape = tf.reshape(
                ...
                shape=[binary_label.get_shape().as_list()[0],
                       binary_label.get_shape().as_list()[1] * binary_label.get_shape().as_list()[2]]) #batch x w x h ?x36x100
            binary_label_reshape = tf.one_hot(binary_label_reshape, depth=5)######## 变为?x3600x5,就可以和decode_logits_reshape算loss了
