    ...
    def encode(self, input_tensor, name):
        ...
        with tf.variable_scope(name):
            ...
            conv_output = self.conv2d(inputdata=dropout_output, out_channel=5,
                                      kernel_size=1, use_bias=True, name='conv_6')  # out_channel=5 --> 6

            print('conv_output_shape: ', conv_output.shape)#(?, 36, 100, 5)

            ret['prob_output'] = tf.image.resize_images(conv_output, [CFG.TRAIN.IMG_HEIGHT, CFG.TRAIN.IMG_WIDTH])

            ### add lane existence prediction branch 添加车道存在预测分支 ###
            ...
            fc_output = self.fullyconnect(relu_output, 4)  # 4
